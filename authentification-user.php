...
if (
  isset($_POST['btn_valider']) &&
  (((isset($_POST['nid_client']) && $_POST['nid_client'] != "") ||
    (isset($_POST['email']) && $_POST['email'] != "")) &&
    (isset($_POST['pin_secret']) && $_POST['pin_secret'] != "")) ||
  ...
  $resultat = $bdd->query("select count(*) as valide,t.* from t_dossier t  where ((nid_pp='" . $use_username . "' or ndel='" . $use_username . "') or  email='" . $email . "') and pin_secret='" . $use_mdp . "' and statut_dossier not in ('Clos_reussi','Clos_echec','Clos_Abandon','	
  ...
  if ($donnee['valide'] == 1) {
    ...
    // Save notification
    if (1) {
      if (($use_username) || ($use_username && $email)) {
        add_notification("t_dossier", $use_username, "Connexion ", "Connexion ", $use_username, "Connexion a MyPASS Online");
      }
      if ($email) {
        add_notification("t_dossier", $email, "Connexion ", "Connexion ", $email, "Connexion a MyPASS Online");
      }
      ...
      if (isset($_POST['pin_secret'])) {
        ...
        header("Location:modification-dossier.php?msg=" . $message . ", Vous êtes connécté a votre compte maintenant. Merci de téléverser vos fichier en cliquant sur le bouton ci-bas" . "&success=ok");
      }
    } else {
    }
    // echo 'sa marche';
  } else {
    header("Location:authentification-user.php?try=ok&tri=2&idr=2");
  }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  if ((empty($_POST["nid_client"]) && $_POST['email'] != "") || (empty($_POST["nid_client"]) && empty($_POST["email"]) && empty($_POST["pin_secret"]))) {
    $nidCLientError = "NID Client est obligatoire";
  } else {
    $nidCLient = test_input($_POST["nid_client"]);
  }
  if ((empty($_POST["email"]) && $_POST['nid_client'] != "") ||  (empty($_POST["nid_client"]) && empty($_POST["email"]) && empty($_POST["pin_secret"]))) {
...
<body>
                ...
                <div class="row">
                  ...
                  <div class="col-lg-12">
                    <div class="white-button first-button scroll-to-section">
                      <a href="ajout-dossier.php"><i class="fa fa-sign-in-alt"></i> Créer Votre Dossier</a>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div class="screen__content">
                    <form class="login" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                      ...
                      <div class="login__field">
                        ...
                        <input type="text" class="login__input" placeholder="NID" name="nid_client" <?php if (!empty($_POST['nid_client'])) {
                                                                                                      echo "value=\"" . $_POST["nid_client"] . "\"";
                                                                                                    } ?>>
                      ...
                      <div class="login__field">
                        ...
                        <input type="text" class="login__input" placeholder="Email" name="email" <?php if (!empty($_POST['email'])) {
                      ...
                      <div class="login__field">
                        ...
                        <input type="password" class="login__input" placeholder="PIN SECRET" name="pin_secret" <?php if (!empty($_POST['pin_secret'])) {
                                                                                                                  echo "value=\"" . $_POST["pin_secret"] . "\"";
                                                                                                                } ?>>
                        <span class="error">* </span>
                        <br>
  ...
  <?php include_once("php/layouts/footer-v2.php");?>

  <?php include_once("php/layouts/script-v2.php");?>
</body>
</html>