...
$get_active_menu="DG";
	$page_titre="Direction Générale > Grand Journal Condensé";
	$page_small_detail="";
	$page_location="Direction Générale > Grand Journal Condensé ";
...
$code_operation="";
$groupage="mois";
$groupage_value=" concat(month(t.creationdate),'-',year(t.creationdate))  ";
...
$param_query=" where 1 ";
	...
	if($ref_compte!=""){
		...
		$param_query=$param_query." and t.ref_compte='".$ref_compte."' ";
		
		}
                switch ($groupage) {
                    case "jour":
                        $groupage_value=" date(t.creationdate) ";
                        
                        break;
                    case "mois":
                        $groupage_value=" concat(year(t.creationdate),'-',month(t.creationdate)) ";
                    
                        break;
                    case "annee":
                        $groupage_value=" year(t.creationdate) ";
                        
                        break;

                    default:
                        
                         $groupage_value=" date(t_creationdate) ";
	...
	if($code_operation!=""){
		...
		$param_query=$param_query." and t.code_operation='".$code_operation."' ";
        ...
        if($ref_agence!=""){
            ...
            $param_query=$param_query." and ref_agence_action=".$ref_agence." ";
...
<?php  include_once("php/print_message.php"); ?>
 ...
 <form  onsubmit="ShowLoading()" class="form-horizontal" action="grand_journal_condensed_general.php"  method="post">
                ...
                <!-- form start -->
                  ...
                  <div class="form-group">
                    ...
                    <div class="form-group">
                      ...
                      <div class="form-group">
                      <label  class="col-sm-2 control-label">Groupage Période </label>
                      <div class="col-sm-4">
                        <select name="groupage" class="form-control select2">
                        <option value="annee" <?php echo ($groupage=='annee') ? 'selected' : ''; ?>>Par Année</option>
                        <option value="mois" <?php echo ($groupage=='mois') ? 'selected' : ''; ?>>Par Mois</option>
                        <option value="jour" <?php echo ($groupage=='jour') ? 'selected' : ''; ?>>Par jour</option>
              ...
              <div class="box box-info">
                <div class="box-header with-border">
                  <h3 class="box-title">Mode Liste</h3>
                    <button onclick="exportTableToCSV('ExportFileGrandJournalCondensed.csv')" class="btn btn-success pull-right" ><i class="fa  fa-file-excel-o fa-download"> Exporter en CSV </i> <i class="fa fa-download"></i></button>                  
                </div><!-- /.box-header -->
                <div class="box-body" style="overflow-y: scroll;">
                  ...
                  <table id="example2" class="table table-bordered table-striped">
                    <thead>
                      ...
                      <th>Periode<?php echo '('.$groupage.')'; ?></th>                 
                      <th>Compte</th>
                    ...
                    <tbody>
                     <?php 
    $sql_select1 = "select ".$groupage_value." as groupage,sum(credit_action) as credit_action,sum(debit_action) as debit_action,code_operation, t.ref_devise, t.ref_user, ref_compte, t_livre_compte.label as compte, t_agence.label as agence,
  sum(solde_parent) as solde_parent   "
        . "from t_grand_journal t join t_livre_compte on (ref_compte=account_no and ref_agence_action=t_livre_compte.ref_agence)  join t_agence on ref_agence_action=id_agence ".$param_query." "
        . "group by code_operation, ref_user, ref_compte, t_livre_compte.label , t_agence.label, t.ref_devise, ".$groupage_value." order  by t_livre_compte.label desc ".$limit;
  ...
  $index=0;
		  ...
		  $index++;
		  	...
		  	?>	
                      ...
                      <td><?php	echo $data1['groupage'] ;?></td> 
                     
                      <td><?php	echo $data1['ref_compte']." ".$data1['compte'];?></td>                      
