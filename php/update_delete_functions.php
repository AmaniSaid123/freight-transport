...
function update_message_souhait($idt_message_souhait, $sujet, $text_sms, $text_mail)
{
    include("php/param.php");

    $sql_search = "update t_message_souhait set txt_sms='" . $text_sms . "', txt_mail='" . $text_mail . "', sujet ='" . $sujet . "' where Idt_message_souhait=" . $idt_message_souhait;
    $sql_query = $bdd->exec($sql_search);
    //echo $sql_search;
...
/** Added : update details prise en charge */
function update_details_zone_charge_front($idt_dossier, $pc_qualite_garant, $pc_lieu_travail_garant, $pc_salaire_parrain, $pc_activite_pro_chk, $pc_activite_pro_nom, $pc_revenu_parrain, $pc_nbre_parcelle, $pc_nbre_vehicule, $numero_telephone_secondaire, $email_secondaire)
{
    include("php/param.php");

    $sql_search = "update t_dossier set pc_qualite_garant='" . $pc_qualite_garant . "',pc_lieu_travail_garant='" . $pc_lieu_travail_garant . "',pc_salaire_parrain='" . $pc_salaire_parrain . "',pc_activite_pro='" . $pc_activite_pro_chk . "',pc_activite_pro_nom='" . $pc_activite_pro_nom . "',pc_revenu_parrain='" . $pc_revenu_parrain . "', pc_nbre_parcelle='" . $pc_nbre_parcelle . "' ,pc_nbre_vehicule='" . $pc_nbre_vehicule . "' ,numero_telephone_secondaire='" . $numero_telephone_secondaire . "' ,email_secondaire='" . $email_secondaire . "'   where idt_dossier=" . $idt_dossier;
    $sql_query = $bdd->exec($sql_search);
    return $sql_query;
}
...
function update_banner($compte, $agence, $price, $edit) {
    include("php/param.php");
    $sql_search = "update t_banners set compte='" . $compte . "', agence='" . $agence . "', price='" . $price . "' where id=" . $edit;
    $sql_query = $bdd->exec($sql_search);
    return $sql_query;
}
function updateEmailPayment($txtMail, $code)
{
    include("param.php");
    $resultat = mysqli_query($bdd_i, "UPDATE `t_email` SET `txt_mail`= '" . $txtMail . "' WHERE `code` = '" . $code . "'");
    return $resultat;
}
?>