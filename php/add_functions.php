...
function add_dossier($nid,$ndel, $identite, $lieu_naissance, $date_naissance, $email, $nbre_enfant_famille, $position_dans_famille, $numero_passport, $date_expiration_pp, $activite_passe_actuelle, $vo_destination, $vo_raison_voyage, $vo_charge_etude_parrain, $vo_ancien_visa, $vo_ancien_visa_comment, $numero_telephone, $vo_refus_visa, $vo_refus_visa_comment, $vo_destination_famille, $vo_destination_comment, $pc_qualite_garant, $pc_lieu_travail_garant, $pc_salaire_parrain, $pc_activite_pro, $pc_activite_pro_nom, $pc_revenu_parrain, $pc_nbre_parcelle, $pc_nbre_vehicule, $statut_dossier, $promoteur_agence, $ref_agence, $pin_secret, $ref_user,$commentaire_client,$identite_pere,$lieu_naissance_pere,$datenaissance_pere,$identite_mere,$lieu_naissance_mere,$datenaissance_mere,$sexe,$adresse,$ville_pays,$domaine_preference, $q_universite, $q_pays,$numero_telephone_secondaire,$email_secondaire) {
    ...
    $sql_query = "INSERT INTO `t_dossier`
(
`ndel`,nid_pp,
`creationdate`,
`identite`,
...
`pin_secret`,
 ...
 adresse,
 ville_pays,
 domaine_preference, q_universite, q_pays,numero_telephone_secondaire,email_secondaire)
VALUES
(
'" . $ndel . "','" . $nid . "',
now(),
...
// new add dossier 
function add_dossier_new ($nid,$ndel, $identite, $lieu_naissance, $date_naissance, $email, $nbre_enfant_famille, $position_dans_famille, $numero_passport, $date_expiration_pp, $activite_passe_actuelle, $vo_destination, $vo_raison_voyage, $vo_charge_etude_parrain, $vo_ancien_visa, $vo_ancien_visa_comment, $numero_telephone, $vo_refus_visa, $vo_refus_visa_comment, $vo_destination_famille, $vo_destination_comment, $pc_qualite_garant, $pc_lieu_travail_garant, $pc_salaire_parrain, $pc_activite_pro, $pc_activite_pro_nom, $pc_revenu_parrain, $pc_nbre_parcelle, $pc_nbre_vehicule, $statut_dossier, $promoteur_agence, $ref_agence, $pin_secret, $ref_user,$commentaire_client,$identite_pere,$lieu_naissance_pere,$datenaissance_pere,$identite_mere,$lieu_naissance_mere,$datenaissance_mere,$sexe,$adresse,$ville_pays,$domaine_preference, $q_universite, $q_pays,$numero_telephone_secondaire,$email_secondaire) {
    include("param.php");
...
`pin_secret`,
 ...
 domaine_preference,
 q_universite,
 q_pays,numero_telephone_secondaire,email_secondaire)
VALUES
(
'" . $ndel . "','" . $nid . "',
now(),
...
function add_dossier_etude($ref_dossier, $annee, $institution, $formation, $niveau, $resultat, $diplome_obtenu, $url, $ville_obtention, $pays_obtention, $ref_user) {
    ...
    $resultat = $bdd->exec("INSERT INTO `t_dossier_etude`
(
`ref_dossier`,
`annee`,
`institution`,
...
function add_dossier_etude_new($ref_dossier, $annee, $institution, $formation, $niveau, $pourcentage, $diplome_obtenu, $url, $ville_obtention, $pays_obtention, $ref_user)
{
    include("param.php");
    foreach ((array) $ref_dossier as $key=>$ref) {
        foreach ((array) $annee as $key=>$ann) {
            if ($diplome_obtenu === "SECONDAIRE") {
            //first
                $niveau = $niveau[$key];
            }
            //second
            elseif ($niveau === "EXETAT") {
                $niveau = $niveau;
            }
            //third
            else {
                $niveau = $niveau[$key];
                $diplome_obtenu =  $diplome_obtenu[$key];
            }
            ...
            `creationdate`,ref_user)
            VALUES
            ('" . $ref . "','" . $annee[$key] . "','" . $institution[$key] . "','" . $formation[$key] . "','" . $niveau . "','" . $pourcentage[$key] . "','" . $diplome_obtenu . "','','" . $ville_obtention[$key]. "','" . $pays_obtention[$key] . "',now(),'" . $ref_user . "')");
        }
    }
    return $resultat;
}
function add_document($ref_dossier, $ref_user, $url_document, $type_fichier,$titre) {
    ...
    $resultat = $bdd->exec("INSERT INTO `t_document_dossier`
(
`ref_dossier`,
`creationdate`,
`ref_user`,
...
/*echo "INSERT INTO `t_document_dossier`
(
`ref_dossier`,
`creationdate`,
`ref_user`,
...
function add_action_request($ref_dossier, $ref_operation, $delai, $mode_reception_1, $mode_reception_2, $mode_reception_3, $mode_reception_4, $notification_sms, $notification_email, $notification_appel, $statut, $ref_requester, $customer_viewable, $commentaire) {
    ...
    $resultat = $bdd->exec("NSERT INTO `t_actions`
(
`ref_dossier`,
`ref_operation`,
`debut_date`,
...
function add_action_info($ref_dossier, $ref_operation, $montant, $devise, $date_paiement,$mode_paiement, $statut,$ref_requester, $customer_viewable, $commentaire,$sms,$mail,$mailSencondaire) {
    include("param.php");
...
function add_banner($compte, $agence, $price)
{
    include("param.php");
    $sql_add_banner = "insert into t_banners(compte,agence,price,creationdate) values('" . $compte . "','" . $agence . "','" . $price . "',now())";
    $sql_query = $bdd->exec($sql_add_banner);
    return $sql_query;
}
function add_journal($userId, $profileId, $date, $arrivingTime, $exitTime, $description)
{
    include("param.php");
    $sql_add_journal = "insert into t_journal_users(user_id,profile_id,date,arriving_time,exit_time,description,holiday_day,creationDate, lastUpdate) values('" . $userId . "','" . $profileId . "','" . $date . "','" . $arrivingTime . "','" . $exitTime . "','" .$description ."',0,now(), now())";
    $sql_query = $bdd->exec($sql_add_journal);
    return $sql_query;
}
function add_journal_holiday($userId, $profileId, $date)
{
    include("param.php");
    $sql_add_journal = "insert into t_journal_users(user_id,profile_id,date,arriving_time,exit_time,description,holiday_day,creationDate, lastUpdate) values('" . $userId . "','" . $profileId . "','" . $date . "',NULL,NULL,NULL,1,now(), now())";
  
    $sql_query = $bdd->exec($sql_add_journal);
    return $sql_query;
}
function add_more_journal($user_id, $more_details, $date)
{
    include("param.php");
    $sql_add_more_journal = "update t_journal_users set more_details='" . $more_details . "' where user_id='" . $user_id . "' and date='" . $date . "'";
    $sql_query = $bdd->exec($sql_add_more_journal);
    return $sql_query;
}
function add_comment_journal($id, $comment, $reaction, $byUser)
{
    include("param.php");
    $sql_add_comment_journal = "update t_journal_users set comment='" . $comment . "', date_comment= now(),comment_by='" . $byUser . "', reaction='" . $reaction . "' where id= $id";
    $sql_query = $bdd->exec($sql_add_comment_journal);
    return $sql_query;
}
...
function insert_journal($iduser,$idprofile ,$dateNow)
{
    include("param.php");
    $insert_journal = 'INSERT INTO `t_journal_users` (`id`, `user_id`, `profile_id`, `date`, `arriving_time`, `exit_time`, `description`, `more_details`, `comment`, `comment_by`, `date_comment`, `reaction`, `holiday_day`,`creationDate`, `lastUpdate`) VALUES (NULL, '.$iduser.', '.$idprofile.', "' . $dateNow . '", NULL , NULL,NULL, NULL, NULL, NULL, NULL, NULL,0, now(), now())';
    $sql_query = $bdd->exec($insert_journal);
    return $sql_query;
}
function addEmailPayment($txt_mail, $code)
{
    include("param.php");

    $resultat = $bdd->exec("INSERT INTO t_email(txt_mail,code,date_creation) VALUES('".$txt_mail."','".$code."',now())");
    return $resultat;
}

function add_online_payment($ref_dossier, $ref_operation, $other_operation, $montant, $full_name, $comment, $email) {
    include("param.php");

    $ref_dossier_value = ($ref_dossier === null || $ref_dossier === '') ? "NULL" : "'" . addslashes($ref_dossier) . "'";
    $ref_operation_value = ($ref_operation === null || $ref_operation === '') ? "NULL" : "'" . addslashes($ref_operation) . "'";

    $query = "INSERT INTO `t_online_payments`
    (`ref_dossier`, `ref_operation`, `other_operation`, `montant`, `full_name`, `comment`, `email`, `created_at`)
    VALUES
    (
    $ref_dossier_value,
    $ref_operation_value,
    '" . addslashes($other_operation) . "',
    '" . $montant . "',
...
function add_contact($name, $email, $subject, $message ,$ref_dossier) {

    include("param.php");
    $ref_dossier_value = ($ref_dossier === null || $ref_dossier === '') ? "NULL" : "'" . addslashes($ref_dossier) . "'";
    $query = "INSERT INTO `t_contacts`
    (`name`, `email`, `subject`, `message`, `created_at`, `ref_dossier`)
    VALUES
    (
    '" . $name . "',
    '" . $email . "',
