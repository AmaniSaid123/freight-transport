...
$get_active_menu = "DG";
$page_titre = "Direction Générale >  Livre Journal";
$page_small_detail = "Livre Journal";
$page_location = "Direction Générale >  Livre Journal";
$ref_agence = '';
//****************location******************
...
if (isset($_POST['submit_add_capture']) && clean_in_text($_POST['libelle']) != "") {
    ...
    $sql_query_capture = mysqli_query($bdd, "select * from t_livre_compte where ref_agence=" . $_SESSION['my_agence'] . " ");
    $row = 0;
...
if (isset($_POST['submit_add_capture_general']) && clean_in_text($_POST['libelle']) != "") {
    ...
    $libelle = clean_in_text($_POST['libelle']);
    $sql_query_capture = mysqli_query($bdd, "select * from t_livre_compte ");
    $row = 0;
...
if (isset($_GET['action_click']) && $_GET['action_click'] == "set_extend" && isset($_GET['find'])) {
    ...
    $feedback_action = set_compte_display_value_dg(clean_in_text($_GET['find']), "Yes");
...
if (isset($_GET['action_click']) && $_GET['action_click'] == "set_hidden" && isset($_GET['find'])) {
    ...
    $feedback_action = set_compte_display_value_dg(clean_in_text($_GET['find']), "No");
    if ($feedback_action == 1) {
        $success = "yes";
...
if (isset($_POST['submit_add_compte']) && $_POST['label'] != "") {
    ...
    if ($feedback_add_account == 1) {
        add_notification("t_user", "NA", "Connexion ", "Compte  " . clean_in_text($_POST['account_no']) . " | Label : " . clean_in_text($_POST['label']) . " | CF : " . clean_in_double($_POST['credit_final']) . " | DF : " . clean_in_double($_POST['debit_final']) . " | C CDF :" . clean_in_double($_POST['credit_cdf']) . " | D CDF :" . clean_in_double($_POST['debit_cdf']) . " | C USD :" . clean_in_double($_POST['credit_usd']) . " | D USD :" . clean_in_double($_POST['debit_usd']) . " | Agence : " . $_SESSION['my_agence'], $use_username, "Ajout Compte dans Livre");
...
$compte = '';
$param = '';
$sql_select1 = "select t.account_no, t.label,  'DG' as agence,sum(credit_final) as credit_final,sum(debit_final) as debit_final,sum(solde_final) as solde_final,ref_devise,display_sub, sum(credit_usd) as credit_usd, sum(debit_usd) as debit_usd, sum(solde_usd) as solde_usd, sum(credit_cdf) as credit_cdf, sum(debit_cdf) as debit_cdf, sum(solde_cdf) as solde_cdf  from t_livre_compte t join t_agence on ref_agence=id_agence where 1  " . $param . " group by t.account_no, t.label,  'DG', ref_devise,display_sub";
...
if (isset($_POST['submit_search'])) {
    ...
    if (isset($_POST['ref_agence']) && $_POST['ref_agence'] != '') {

        $sql_select1 = "select t.account_no, t.label,  t_agence.label as agence,credit_final,debit_final,solde_final,ref_devise,display_sub, credit_usd, debit_usd, solde_usd, credit_cdf, debit_cdf, solde_cdf  from t_livre_compte t join t_agence on ref_agence=id_agence where ref_agence=" . $ref_agence . " " . $param . " group by t.account_no, t.label,  'DG', ref_devise,display_sub";
    } else {
...
<div class="content-wrapper">
    ...
    <section class="content">
        ...
        <form onsubmit="ShowLoading()" class="form-horizontal" action="livre_compte_dg.php" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <select name="ref_agence" class="form-control select2">
                                <option value="" <?php echo ($ref_agence == '') ? 'selected' : ''; ?>>Toute l'entreprise</option>
        ...
        <form onsubmit="ShowLoading()" action="#" method="post">
            <div class="box box-info">
                <div class="box-header with-border">
                    ...
                    <button onclick="exportTableToCSV('ExportFileLivreCompteDG.csv')" class="btn btn-success pull-right"><i class="fa  fa-file-excel-o fa-download"> Exporter en CSV </i> <i class="fa fa-download"></i></button>
                </div><!-- /.box-header -->
                            ...
                            ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            if ($data1['display_sub'] == "Yes") {
                                            ?>
                                                <a href="livre_compte_dg.php?bantalaium=jakudkiusjsjjs8568rgfddghjjkjh52s852z685s2zz2&xdeligirium=uejdid856712s55z82z55d5s2s52s45zoi&action_click=set_hidden&find=<?php echo $data1['account_no']; ?>" onClick="return confirm('Cette action va cacher les autres devises du compte, Vous confirmez')"><i class="fa fa-fw  fa-arrow-right"></i><i class="fa fa-fw  fa-refresh"></i> </a>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            ?>
                                                <a href="livre_compte_dg.php?bantalaium=jakudkiusjsjjs8568rgfddghjjkjh52s852z685s2zz2&xdeligirium=uejdid856712s55z82z55d5s2s52s45zoi&action_click=set_extend&find=<?php echo $data1['account_no']; ?>" onClick="return confirm('Cette action va affiché Toutes le devises du compte, Vous confirmez')"><i class="fa fa-fw  fa-arrow-right"></i><i class="fa fa-fw  fa-refresh"></i> </a>
