...
if (isset($_GET["del"])  && get_access(28, $_SESSION['my_idprofile']) == 1) {
  ...
  $action_click = "1";
  $feedback_deletion = delete_dossier($idt_dossier, $action_click);

  if ($feedback_deletion == 1) {

    $add_request_info = add_action_info($data_dossier_temp['idt_dossier'], 29, "", "", "", "", "Valide", $_SESSION['my_username'], "Oui", "Suppression Dossier ", 0, 0, 0);
    //$data_dossier_temp = get_dossier_data($edit);
    $success = "yes";
    $success_message = "Le dossier a �t� supprim� avec succ�s";
    //header("Location: liste_dossier.php");
...
if (isset($_GET["res"])  && get_access(28, $_SESSION['my_idprofile']) == 1) { //ON FAIT LA RESTAURATION
  $idt_dossier = clean_in_integer($_GET["res"]);
  $data_dossier_temp = get_dossier_data($idt_dossier);
  ...
  if ($feedback_deletion == 1) {
    ...
    $add_request_info = add_action_info($data_dossier_temp['idt_dossier'], 29, "", "", "", "", "Valide", $_SESSION['my_username'], "Oui", "Resturation Dossier ", 0, 0, 0);
    //$data_dossier_temp = get_dossier_data($edit);
    $success = "yes";
    $success_message = "Le dossier a EtE restaurE avec succes";
  } else {
    $error = "yes";
    $error_message = "Erreur de la restauration du dossier";
  }
}
    ...
    <div class="box box-info">
      ...
      <div class="box-body" style="overflow-y: scroll;">
        <form action="liste_dossier.php" method="POST">
          <div class="form-group">
            <?php if (get_access(60, $_SESSION['my_idprofile']) == 1) { ?>
              <label style="color: red;">Afficher le(s) Dossier(s) supprimE(s) ?</label><br>
              <button type="submit" value="yes" name="btn_action" class="btn btn-default">Dossiers non supprimEs</button>
              <button type="submit" value="no" name="btn_action" class="btn btn-warning">Dossiers supprimEs</button>
        ...
        <table id="example2" class="table table-bordered table-striped">
          ...
          <tbody>
            <?php if (isset($_POST['btn_action']) && $_POST['btn_action'] == "no") : ?>

              <?php
              $param_query = " where deletion_statut=1  order by idt_dossier desc limit 0,500";
              ...
              ?>
                <tr style="background: #ffc107;">
                  ...
                  <td>
                    ...
                    ?>
                      <?php echo $data1['numero_telephone']; ?>
                    <?php
                    } else {
                    ?>
                      <?php echo hide_mobile_no($data1['numero_telephone']); ?>
                  ...
                  <td>
                    ...
                    ?>
                      <?php echo obfuscate_email($data1['email']); ?>
                    <?php
                    }
                    ?>
                  </td>
                  <td>
                    ...
                    if (get_access(62, $_SESSION['my_idprofile']) == 1) {
                    ?>
                      <?php echo $data1['email_secondaire']; ?>
                    <?php
                    } else {
                    ?>
                      <?php echo obfuscate_email($data1['email_secondaire']); ?>
                    <?php
                    }
                    ?>
                       <td>
                    ...
                    if (get_access(62, $_SESSION['my_idprofile']) == 1) {
                    ?>
                      <?php echo $data1['numero_telephone_secondaire']; ?>
                    <?php
                    } else {
                    ?>
                      <?php echo hide_mobile_no($data1['numero_telephone_secondaire']); ?>
                  ...
                  <td>
                    ...
                    ?>
                      <a href="liste_dossier.php?res=<?php echo $data1['idt_dossier']; ?>"><i class="fa fa-fw  fa-upload" onClick="return confirm('Cette action va restaurer le dossier, Veuillez confirmer?')"></i> </a>
              ...
              <?php } ?>
            <?php else : ?>
              <!--On veut afficher les dossiers supprimEs-->
              <?php
              $sql_select1 = 'select t.* , t_agence.label as agence from t_dossier t join t_agence on ref_agence=id_agence ' . $param_query . " ";
              $sql_result1 = $bdd->query($sql_select1);
              //echo $sql_select1;
