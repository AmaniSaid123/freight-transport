<?php
return [
    // menu
    'home' => 'Accueil',
    'tracking' => 'Suivre mon Colis',
    'about' => 'À propos',
    'services' => 'Nos services',
    'testimonial' => 'Témoignages',
    'blog' => 'Actualités',
    'contact' => 'Nos contacts et entrepôts',
    'contact_us' => 'Nos contacts',
    'language' => 'Langue',
    'french' => 'Français',
    'english' => 'Anglais',
    'send_parcel' => 'Envoyer un colis',
    'who_are_you' => 'Qui sommes-nous',
    'values' => 'Nos valeurs',
    'proceed' => 'Comment procéder',
    'prices' => 'Nos prix',
    'communication_tracking' => 'Communication et Suivi des expéditions',
    'routes' => 'Nos routes logistiques',
    'routes_sa_menu' => 'De Johannesburg vers Kinshasa et Lubumbashi (avion ou route)',
    'routes_china_menu' => 'De la Chine vers la RDC et l\'Afrique du Sud par bateau',
    'routes_sa_page_title' => 'De Johannesburg vers Kinshasa et Lubumbashi par avion ou par route',
    'routes_china_page_title' => 'De la Chine vers la RDC et l\'Afrique du Sud par bateau',
    'tracking_parcel' => 'Suivi',
    // communication page
    'communication_title' => 'Communication et Suivi',
    'communication_intro' => 'TCC maintient un contact régulier avec ses clients pour garantir une expérience transparente et fiable tout au long du processus d\'expédition.',
    'communication_card_price_title' => 'Prix du Transport',
    'communication_card_price_desc' => 'Nous vous communiquons rapidement et clairement le prix du transport dès la réception de vos colis dans nos entrepôts. Aucun coût caché - tout est transparent.',
    'communication_card_price_highlight' => 'Devis détaillé et compétitif',
    'communication_card_tracking_title' => 'Suivi du Colis',
    'communication_card_tracking_desc' => 'Suivez votre colis en temps réel tout au long de son transit. Nous vous fournissons des mises à jour régulières sur la localisation et le statut de votre envoi.',
    'communication_card_tracking_highlight' => 'Mises à jour en temps réel',
    'communication_card_delivery_title' => 'Processus de Livraison',
    'communication_card_delivery_desc' => 'Nous vous informons à chaque étape du processus de livraison, de l\'expédition à la réception finale dans votre ville.',
    'communication_contact_whatsapp' => 'WhatsApp',
    'communication_contact_email' => 'E-mail',
    'communication_timeline_title' => 'Notre Processus de Communication',
    'communication_step_reception' => 'Réception du Colis',
    'communication_step_pricing' => 'Calcul du Prix',
    'communication_step_shipping' => 'Expédition',
    'communication_step_tracking' => 'Suivi en Transit',
    'communication_step_delivery' => 'Livraison Finale',

    // carousel
    'carousel-1' => 'Agence de fret maritime et aérien',
    'carousel-text-1' => 'La meilleure solution pour votre fret aérien de Johannesburg à la Rép. dém. du Congo et maritime de Chine à la Rép. dém. du Congo et à Johannesburg.',
    'carousel-description-1' => 'Faites livrer vos colis à notre entrepôt en Chine et recevez les à notre entrepôt dans votre ville ',

    'carousel-2' => 'La joie de recevoir votre colis en bonne et due forme',
    'carousel-text-2' => 'Le bonheur, c’est aussi lorsque vous ouvrez votre carton et y trouvez votre colis en bon état',
    'carousel-description-2' => 'TCC fait de son mieux d’offrir sécurité sur des colis même fragiles de sorte que vous receviez votre colis sans dommage. Faites savoir à vos fournisseurs de coller l’étiquette FRAGILE sur vos colis qui sont fragiles, afin de nous permettre de les transporter avec attention supplémentaire.',

    // Qui nous sommes
    'who_are_we_desc_1' => "Trusted Cargo Company (TCC) est un fournisseur fiable de solutions logistiques complètes, spécialisé dans le fret aérien, maritime et routier. Nous nous distinguons par la fiabilité de nos services de livraison de colis et notre capacité à connecter de manière fluide les centres névralgiques du commerce mondial.",

    'who_are_we_desc_2' => "Nos Principales Routes et Destinations",

    'who_are_we_desc_3' => "Nous facilitons des services de fret spécialisés reliant les principaux lieux suivants, garantissant des opérations de transport très efficaces :",

    'who_are_we_desc_4' => "TCC communique régulièrement avec ses clients via email et Whatsapp afin de leur communiquer le prix,
    le suivi du colis en cours de route ainsi que le processus de livraison à Johannesburg, Kinshasa ,Lubumbashi ou Kolwezi.",

    'who_are_we_route_sa_title' => "Depuis l'Afrique du Sud par avion et par route",
    'who_are_we_route_sa_origin' => "Entrepôt d'Origine : Johannesburg.",
    'who_are_we_route_sa_dest_title' => "Centres de Distribution de Destination :",
    'who_are_we_route_sa_dest_1' => "Kinshasa en RDC (par avion uniquement)",
    'who_are_we_route_sa_dest_2' => "Lubumbashi en RDC (par avion et par route)",
    'who_are_we_route_china_title' => "Depuis la Chine par fret maritime",
    'who_are_we_route_china_origin' => "Entrepôts d'Origine : Shenzhen, Guangzhou, Yiwu et autres lieux.",
    'who_are_we_route_china_dest_title' => "Centres de Distribution de Destination :",
    'who_are_we_route_china_dest_1' => "Johannesburg, Afrique du Sud",
    'who_are_we_route_china_dest_2' => "Kinshasa, République Démocratique du Congo (RDC)",
    'who_are_we_route_china_dest_3' => "Lubumbashi, RDC",
    'who_are_we_route_china_dest_4' => "Kolwezi, RDC",

    // tracking
    'tracking_title' => 'Suivi mon colis',
    'tracking_text' => 'Suivez votre colis en temps réel',
    'tracking_description' => 'Insérez votre numéro de suivi ci-dessous pour suivre votre colis. Notre système de suivi vous informe du départ de votre colis de notre entrepôt de Chine jusqu\'à son arrivée à notre entrepôt de votre ville ou sa livraison.',
    'tracking_button' => 'Suivre',
    'tracking_placeholder' => 'Entrez votre numéro de suivi',

    // about
    'about_title' => 'À propos de nous',
    'about_text' => 'Votre partenaire de confiance pour le transport de cargo international',
    'about_description' => 'TCC est spécialisée dans le fret aérien et maritime, reliant la Chine à l\'Afrique avec fiabilité et rapidité. Nous gérons l\'acheminement complet de vos marchandises, de la réception dans nos entrepôts jusqu\'à la collecte ou la livraison chez vous',
    'about_list_1' => 'Expertise en fret aérien et maritime',
    'about_list_2' => 'Suivi en temps réel de vos expéditions',
    'about_list_3' => 'Services personnalisés selon vos besoins',
    'about_route_china_title' => 'Depuis la Chine par fret maritime',
    'about_route_sa_title' => "Depuis l'Afrique du Sud",
    'about_button' => 'Découvrir nos services',

    // blog
    'blog_title' => 'Notre blog',
    'blog_text' => 'Facilitation pour différents types de transport de vos colis',
    'blog_description' => 'Vos colis peuvent être transportés par avion, par bateau tout comme par véhicules lorsqu’il s’agit de vous le livrer à domicile',
    'blog_content_title_1' => 'Par Avion (Fret aérien)',
    'blog_content_description_1' => 'Commandez en ligne et utilisez notre entrepôt de Johannesburg comme adresse de livraison. Nous réceptionnerons vos colis et les expédierons par avion jusqu\'à votre ville en RDC, où vous pourrez ensuite les récupérer dans notre entrepôt local.
    Consultez la section <a href="/contact">Nos contacts et entrepôts</a> pour obtenir des instructions et commencer dès aujourd\'hui.',
    'blog_content_title_2' => 'Par bateau (Fret maritime)',
    'blog_content_description_2' => 'Veuillez livrer vos colis à notre entrepôt en Chine sur base des indications que vous trouverez dans la rubrique <a href="/contact">Nos contacts et entrepôts</a>
    et sélectionnez l’image qui convient à votre ville de destination finale, et envoyez cette image  à votre fournisseur en Chine et précisez lui que vous voulez
     le fret maritime pour votre colis. Faites nous un message WhatsApp ou email nous informant que vous faites livrer vos colis en Chine pour le fret maritime.
    Créez aussi votre dossier MyFret pour nous permettre de vous tenir informé étape par étape sur votre colis en route vers votre ville de destination finale.',
    'blog_content_title_3' => 'Par véhicule',
    'blog_content_description_3' => 'Une fois que votre colis arrive à notre entrepôt dans votre ville de destination finale, nous pouvons, si vous le désirez, vous le livrer à domicile
    ou à l’adresse que vous pouvez nous indiquer. Pour cela, nous vous communiquerons le tarif exact après évaluation de la quantité des colis et de
    l’adresse de livraison que vous nous auriez donnée.',
    // process
    'text_process' => 'Comment procéder ?',
    // values (page keys)
    'values_hero' => 'Nos Valeurs : Intégrité et Transparence',
    'values_card1_title' => 'Information complète',
    'values_card1_desc' => 'Nous veillons à ce que le client soit informé à chaque étape, voilà pourquoi nous demandons ses détails de contact.',
    'values_card2_title' => 'Facturation équitable',
    'values_card2_desc' => 'Nous nous assurons que chaque paiement correspond exactement aux services sollicités, en évitant les surcoûts inutiles.',
    'values_card3_title' => 'Calcul précis',
    'values_card3_desc' => 'Le prix du transport est déterminé par le poids, le volume, la nature du colis, la destination finale et parfois les frais de douane.',
    'values_commitment_title' => 'Engagement TCC',
    'values_commitment_paragraph' => 'TCC s\'engage à fournir un service basé sur l\'intégrité, la vérité et des prix raisonnables. Notre devoir est d\'appliquer un calcul exact après évaluation de tous ces paramètres.',
    // values
    'values_descritption' => 'TCC est engagée à offrir un service basé sur l\'intégrité, la vérité et les prix raisonnables.
                    Nous faisons de notre mieux pour que le client soit informé à chaque étape de l\'évolution de son
                    dossier.
                    Nous faisons de notre mieux pour que chaque paiement du client puisse équivaloir aux services
                    sollicités. Nous évitons que le client se retrouve à payer au-delà du service sollicité, ni à
                    payer pour un service non consommé. 
                    Étant donné que le poids, le volume, la nature du colis, la ville de
                    destination finale et parfois la douane déterminent le prix du transport, et il est de notre
                    devoir de ne faire payer au client que le calcul exact du service après adéquate évaluation de tous ces
                    paramètres.',
    //services
    'services_sub_title' => 'Nos Services',
    'services_hero_title' => 'Des solutions logistiques fiables et sur mesure',
    'services_description' => "Nous vous offrons un ensemble complet de services pour expédier vos marchandises en toute sécurité et efficacité. Du fret aérien et maritime à l’entreposage, en passant par le suivi en temps réel, nous vous accompagnons à chaque étape.",

    'service1_title' => 'Fret Aérien',
    'service1_desc' => 'Transport aérien rapide de Johannesburg à Kinshasa et Lubumbashi.',

    'service2_title' => 'Fret Maritime',
    'service2_desc' => 'Expédition économique par bateau pour petits ou gros volumes des colis.',

    'service3_title' => 'Entreposage',
    'service3_desc' => 'Stockage sécurisé et consolidation de vos commandes.',

    'service4_title' => 'Suivi en Temps Réel',
    'service4_desc' => 'Information continue du départ à la livraison, via email & WhatsApp.',

    'service5_title' => 'Évaluation & Devis',
    'service5_desc' => 'Évaluation rapide et précise des coûts de transport avec devis clair et détaillé.',

    'service6_title' => 'Assistance à l’Achat',
    'service6_desc' => 'Instructions claires pour vos vendeurs afin de livrer correctement.',

    'service7_title' => 'Emballage & Préparation',
    'service7_desc' => 'Emballage sécurisé pour protéger vos marchandises pendant le transport.',

    'service8_title' => 'Assurance Transport',
    'service8_desc' => 'Couverture contre les pertes ou dommages durant l’acheminement.',

    'more_details' => "Plus de détails",

    //create folder
    'full_name' => 'Noms complet du client',
    'phone' => 'Numéro de téléphone',
    'email' => 'Adresse email',
    'address' => 'Adresse du domicile',
    // send parcel
    'send_parcel_subtitle' => 'Planifiez vos envois en quelques clics, ajoutez plusieurs expéditions et suivez-les facilement.',
    'send_parcel_step1_title' => 'Coordonnées',
    'send_parcel_step1_desc' => 'Nous utiliserons ces informations pour vous tenir informé à chaque étape.',
    'send_parcel_step2_title' => 'Détails d\'expédition',
    'send_parcel_step2_desc' => 'Ajoutez les trajets, précisez vos colis et supprimez ceux en trop.',
    'express_shipping' => 'Envoi express',
    'expedition_label' => 'Expédition',
    'add_expedition' => 'Ajouter une expédition',
    'remove_expedition' => 'Supprimer l\'expédition',
    'reset' => 'Réinitialiser',
    'send' => 'Envoyer',
    'required_full_name' => 'Le nom complet est requis.',
    'required_phone' => 'Le téléphone (indicatif et numéro) est requis.',
    'required_email' => 'L\'email est requis.',
    'required_address' => 'L\'adresse est requise.',
    'required_origin' => 'L\'origine est requise.',
    'required_destination' => 'La destination est requise.',
    'required_description' => 'La description est requise.',
    'client_toggle_label' => 'Vous êtes client',
    'client_toggle_hint' => 'Mode client : seul l\'email est nécessaire. Mode interne : formulaire complet.',
    // footer
    'footer_company_title' => 'TCC',
    'footer_company_desc' => 'TCC Logistics est spécialisé dans le fret aérien et maritime, reliant la Chine à l\'Afrique du Sud et à la République démocratique du Congo. Nous gérons l\'acheminement complet de vos marchandises, de la réception dans nos entrepôts en Chine jusqu\'à la collecte ou la livraison dans votre ville.',
    'footer_quick_links_title' => 'Liens rapides',
    'footer_quick_about' => 'À propos',
    'footer_quick_contacts' => 'Nos contacts et entrepôts',
    'footer_quick_pay' => 'Payer votre envoi',
    'footer_quick_terms' => 'Nos Termes et Conditions',
    'footer_quick_values' => 'Nos valeurs',
    'footer_contact_title' => 'Informations de contact',
    'footer_contact_des_kwz' => 'Le retrait des colis pour Kolwezi se fait auprès de nos partenaires chinois sur place dans
                        Kolwezi. L’adresse sera communiquée au cas par cas lors du retrait. Toute assistance nécessaire
                        pour les clients de Kolwezi doit se faire avec notre bureau de Lubumbashi.',
    'footer_badge_depot' => 'Dépôt & retrait de colis uniquement',
    'footer_note_depot' => 'Adresse réservée au dépôt/retrait de colis. Aucune démarche administrative sur place.',
    'footer_badge_admin' => 'Accueil & Administration (sur rendez-vous)',
    'footer_note_admin' => 'Veuillez prendre rendez-vous avant de vous présenter à nos bureaux.',
    // prices page
    'prices_title' => 'Nos prix et nos délais de livraison',
    'prices_intro_full' => 'Le prix du fret est volatile et dépend de plusieurs facteurs : la nature du produit (standard, dangereux, sensible, électrique, batteries, liquides, etc.), les frais de douane, la destination finale, et le type de fret (aérien ou maritime). Voici nos tarifs standards indicatifs pour le cargo général. Le prix exact sera fixé après évaluation de votre colis en Chine ou à Johannesburg (pour l’envoi vers la RDC), selon sa nature et ses paramètres. Pour Johannesburg-RDC, les prix des téléphones, ordinateurs et documents seront communiqués au cas par cas sur demande du client.',
    'prices_intro_lead' => 'Le prix du fret est volatile et dépend de plusieurs facteurs :',
    'prices_intro_factor_product' => 'La nature du produit (standard, dangereux, sensible, électrique, batteries, liquides, etc.).',
    'prices_intro_factor_customs' => 'Les frais de douane.',
    'prices_intro_factor_destination' => 'La destination finale.',
    'prices_intro_factor_mode' => 'Le type de fret (aérien ou maritime).',
    'prices_intro_note' => 'Voici nos tarifs standards indicatifs pour le cargo général. Le prix exact sera fixé après évaluation de votre colis en Chine ou à Johannesburg (pour l’envoi vers la RDC), selon sa nature et ses paramètres. Pour Johannesburg-RDC, les prix des téléphones, ordinateurs et documents seront communiqués au cas par cas sur demande du client.',
    'prices_table_destination' => 'Destination',
    'prices_table_type' => 'Type de Fret',
    'prices_table_price' => 'Prix',
    'prices_table_leadtime' => 'Délais',
    'prices_table_notes' => 'Notes',
    'prices_customs_included' => 'Douane incluse',
    'prices_comparison_subtitle' => 'Trajets depuis Johannesburg en premier, suivis des trajets depuis la Chine',
    'prices_disclaimer' => 'Note importante : Les frais standards de douane sont inclus dans tous les prix indiqués.',
    'prices_section_from_jhb' => 'Trajets depuis Johannesburg, Afrique du Sud',
    'prices_section_from_china' => 'Trajets depuis la Chine',
    'prices_table_origin' => 'Origine',
    'prices_origin_jhb' => 'Johannesburg, Afrique du Sud',
    'prices_origin_china' => 'Chine',
    'prices_note_pay_jhb_kins' => 'Paiement possible à Johannesburg lors de l\'expédition ou à Kinshasa lors de la collecte',
    'prices_note_pay_jhb_lub' => 'Paiement possible à Johannesburg lors de l\'expédition ou à Lubumbashi lors de la collecte',
    'prices_note_road_lub' => 'Option terrestre depuis l\'Afrique du Sud',

    'prices_row_jhb_destination' => 'Johannesburg',
    'prices_row_jhb_sea_type' => 'Fret Maritime',
    'prices_row_jhb_sea_price' => 'À partir de R 4,999 par CBM',
    'prices_row_jhb_sea_leadtime' => '35 à 45 jours',
    'prices_row_jhb_sea_notes' => 'Dédouanement inclus',

    'prices_row_kins_destination' => 'Kinshasa',
    'prices_row_kins_air_type' => 'Fret Aérien',
    'prices_row_kins_air_price' => 'À partir de 10 USD (R175) par Kg',
    'prices_row_kins_air_leadtime' => '3 à 10 jours',
    'prices_row_kins_air_notes' => 'Dédouanement inclus',

    'prices_row_kins_sea_type' => 'Fret Maritime',
    'prices_row_kins_sea_price' => 'À partir de 545 USD par CBM',
    'prices_row_kins_sea_leadtime' => '50 à 60 jours',
    'prices_row_kins_sea_notes' => 'Dédouanement inclus',


    'prices_row_lumb_destination' => 'Lubumbashi',
    'prices_row_lumb_air_type' => 'Fret Aérien',
    'prices_row_lumb_air_price' => 'À partir de 10 USD (R175) par Kg',
    'prices_row_lumb_air_leadtime' => '3 à 10 jours',
    'prices_row_lumb_air_notes' => 'Dédouanement inclus',

    'prices_row_lumb_sea_type' => 'Fret Maritime',
    'prices_row_lumb_sea_price' => 'À partir de 545 USD par CBM',
    'prices_row_lumb_sea_leadtime' => '50 à 60 jours',
    'prices_row_lumb_sea_notes' => 'Dédouanement inclus',
    'prices_row_lumb_road_type' => 'Fret Routier',
    'prices_row_lumb_road_price' => 'À partir de 3 USD (R55) par Kg',
    'prices_row_lumb_road_leadtime' => '5 à 12 jours',
    'prices_row_lumb_road_notes' => 'Dédouanement inclus',

    'prices_row_kwz_destination' => 'Kolwezi',
    'prices_row_kwz_sea_type' => 'Fret Maritime',
    'prices_row_kwz_sea_price' => 'À partir de 545 USD par CBM',
    'prices_row_kwz_sea_leadtime' => '50 à 60 jours',
    'prices_row_kwz_sea_notes' => 'Dédouanement inclus',
    'prices_detail_note' => 'Prix détaillé disponible dans le document PDF',

    // process page
    'process_hero_title' => 'Simple. Voici la procédure à suivre :',
    'process_step1_title' => 'Première étape : Création de votre dossier',
    'process_step1_paragraph' => 'Ici dans notre site web, veuillez cliquer sur "Créer votre dossier fret" ensuite remplissez le petit formulaire en ligne qui s’affiche afin que nous puissions avoir vos détails.',
    'process_step1_detail' => 'Ces détails nous permettront de vous notifier sur toutes les étapes de votre fret maritime ou aérien.',
    'process_step1_note' => 'Remplir ce formulaire ne vous prendra que 60 secondes.',
    'process_step1_button' => 'Créer votre dossier fret',
    'process_step1_bullet1' => 'Cliquez sur "Créer votre dossier fret" sur notre site web.',
    'process_step1_bullet2' => 'Remplissez le formulaire en ligne (environ 60 secondes) pour nous fournir vos coordonnées. Ces informations sont essentielles pour vous notifier à chaque étape de votre fret.',

    'process_step2_title' => 'Deuxième étape : Demande des adresses et instructions',
    'process_step2_paragraph' => 'Par WhatsApp ou par mail à admin@trustedcargo.co.za, faites nous un message simple demandant nos adresses en Chine (si vous ne les avez pas encore prises toujours ici dans notre site web, rubrique <a href="/contact">Nos contacts et entrepôts</a>) et précisez nous si vous voulez le fret maritime ou aérien, et dites nous la ville de destination finale de votre colis, nous vous enverrons un fichier format photo contenant TOUS les détails dont votre fournisseur en Chine a besoin pour livrer vos colis chez nous. Si vous envoyez cette photo à votre fournisseur, c’est suffisant pour qu’il sache où et comment il va livrer vos colis à notre entrepôt en Chine.',
    'process_step2_bullet1' => 'Contactez-nous par WhatsApp ou e-mail (admin@trustedcargo.co.za) pour obtenir nos adresses de Chine ou d’Afrique du Sud ou obtenez-les dans la rubrique « <a href="/contact">Nos Contacts et Entrepôts</a> » sur notre site web.',
    'process_step2_bullet2' => 'Dites à votre fournisseur d’indiquer clairement :',
    'process_step2_bullet2_option1' => 'Si vous souhaitez le fret maritime ou aérien (pour la Chine).',
    'process_step2_bullet2_option2' => 'La ville de destination finale de votre colis.',
    'process_step2_bullet3' => 'Nous vous enverrons un fichier photo (ou vous pouvez le télécharger vous-même sur notre site) contenant toutes les instructions nécessaires à transmettre à votre fournisseur en Chine pour la livraison de vos colis à notre entrepôt.',
    'process_step2_bullet4' => 'Les instructions pour les colis allant de Johannesburg vers Kinshasa ou Lubumbashi sont également disponibles dans la rubrique « <a href="/contact">Nos contacts et entrepôts</a> » pour insérer les détails de livraison lors de vos achats en ligne sur des sites sud-africains ou étrangers qui livrent en Afrique du Sud.',
    'process_contact_whatsapp' => 'WhatsApp',
    'process_note_label' => 'Note :',
    'process_note_text' => 'Dans la rubrique <a href="/contact">Nos contacts et entrepôts</a>, vous trouverez les détails en format photo qu’il faut télécharger pour envoyer à votre fournisseur en Chine. Ca contient toutes les orientations dont votre fournisseur a besoin pour livrer vos colis à notre entrepôt en Chine selon qu’il s\'agit du fret aérien ou maritime et selon la destination finale de votre colis.',
    'process_step2_button' => 'Voir nos contacts et entrepôts',
    'process_china_title' => "Comment ça marche pour l'expédition de Chine vers l'Afrique du Sud et la RDC",
    'process_china_intro' => "Si vous êtes à Johannesburg, Kinshasa, Lubumbashi ou Kolwezi, vous pouvez commander vos produits sur des sites chinois (comme Alibaba et plein d'autres) et les faire livrer dans nos entrepôts en Chine (voir <a href=\"/contact\">Nos contacts et entrepôts</a>).",
    'process_china_step1_title' => 'Réception en Chine :',
    'process_china_step1_desc' => 'Vos colis arrivent dans nos entrepôts chinois, vous avez les adresses et les détails nécessaires dans la section « <a href="/contact">Nos contacts et entrepôts</a> ».',
    'process_china_step2_title' => 'Devis Transport :',
    'process_china_step2_desc' => "On évalue vos colis et on vous donne le prix du transport.",
    'process_china_step3_title' => 'Envoi :',
    'process_china_step3_desc' => 'Selon votre mode de transport (par avion ou par bateau), on expédie les colis de Chine directement vers votre ville : Johannesburg, Kinshasa, Lubumbashi ou Kolwezi.',

    'process_sa_title' => "Procédure d'Expédition : De l'Afrique du Sud vers Kinshasa ou Lubumbashi (RDC)",
    'process_sa_intro' => "Vous avez la possibilité d'acheter vos articles sur des plateformes sud-africaines ou chinoises et de les faire livrer directement à notre entrepôt à Johannesburg. Les instructions précises pour la livraison sont disponibles dans la section \"<a href=\"/contact\">Nos contacts et entrepôts</a>\".",
    'process_sa_steps_intro' => "Voici les étapes clés de notre service d’envoi de Johannesburg vers la RDC :",
    'process_sa_step1_title' => 'Achat de vos produits ou Envoi de colis personnels à notre entrepôt de Johannesburg :',
    'process_sa_step1_desc' => 'Nous vous offrons une solution simple et sécurisée pour la gestion de vos achats en ligne (sur des sites e-commerce livrant en Afrique du Sud) et de vos colis personnels avant leur réexpédition.',
    'process_sa_step1_purchase_title' => 'Pour vos achats en ligne (Afrique du Sud et international) :',
    'process_sa_step1_purchase_desc' => "Utilisez simplement l'adresse de notre entrepôt de Johannesburg comme adresse de livraison. Nous prenons en charge la réception, l'enregistrement, et la préparation de vos colis pour leur envoi final. C'est la méthode idéale pour regrouper plusieurs commandes.",
    'process_sa_step1_personal_title' => 'Pour les colis personnels (déposés par un proche ou un tiers) :',
    'process_sa_step1_personal_desc' => "Deux options s'offrent à vous pour acheminer ces colis à notre entrepôt :",
    'process_sa_step1_direct_title' => 'Dépôt Direct :',
    'process_sa_step1_direct_desc' => "La personne dépose le colis directement à notre entrepôt de Johannesburg. Il est conseillé de prendre rendez-vous ou de venir durant nos heures d'ouverture.",
    'process_sa_step1_courier_title' => 'Envoi par Coursier Express :',
    'process_sa_step1_courier_desc' => "Vous pouvez utiliser des services de livraison rapide comme Uber Package ou Bolt Package. Assurez-vous de bien spécifier le nom du destinataire ou une référence pour faciliter l'identification.",
    'process_sa_step2_title' => 'Information Cruciale :',
    'process_sa_step2_desc' => "Il est impératif de nous informer immédiatement par WhatsApp ou e-mail dès qu'un colis est en route (achat en ligne ou envoi par un tiers) ! Veuillez nous fournir le numéro de suivi (si disponible), une brève description, et la ville de destination finale (Kinshasa ou Lubumbashi). Cette démarche garantit un traitement rapide et précis de votre marchandise. Notre objectif est de simplifier toute votre logistique, de l'Afrique du Sud jusqu'à votre ville !",
    'process_sa_step3_title' => 'Réception et Horaires à Johannesburg :',
    'process_sa_step3_desc' => 'Notre entrepôt réceptionne vos livraisons quotidiennement, y compris les dimanches et jours fériés, de 07h00 à 20h30.',
    'process_sa_step4_title' => 'Évaluation et Devis :',
    'process_sa_step4_desc' => 'Une fois les colis réceptionnés, nous procédons à leur évaluation pour vous communiquer le prix du transport. Ils sont ensuite planifiés pour la prochaine expédition vers votre destination finale en RDC (Kinshasa ou Lubumbashi).',
    'process_sa_step5_title' => 'Options de Paiement :',
    'process_sa_step5_desc' => 'Le règlement de vos frais d\'expédition peut se faire en ligne via la rubrique <a href="payment.php">"Paiement"</a> de notre site web ou en espèces à notre entrepôt lors du dépôt/envoi du colis ou lors de la réception du colis dans votre ville.',
    'process_tab_china' => 'Chine vers Afrique',
    'process_tab_sa' => 'Afrique du Sud vers RDC',
    'process_transport_air_desc' => 'Livraison rapide par fret aérien.',
    'process_transport_sea_desc' => 'Solution économique par fret maritime.',
    'process_sa_info_list' => 'Numéro de suivi, description et ville de destination finale (Kinshasa ou Lubumbashi).',
    'process_payment_online' => 'Paiement en ligne',
    'process_payment_online_desc' => 'Via la rubrique <a href="payment.php">"Paiement"</a> du site.',
    'process_payment_cash' => 'Paiement en espèces',
    'process_payment_cash_desc' => 'Lors du dépôt ou à la réception du colis.',
    'process_arrival_title' => "À l'arrivée du colis à notre entrepôt (Chine ou Johannesburg)",
    'process_arrival_eval' => 'Nous évaluons le colis.',
    'process_arrival_comm' => 'Nous vous communiquons :',
    'process_arrival_tracking' => 'Le numéro de suivi ou de référence du colis.',
    'process_arrival_price' => 'Le prix à payer.',
    'process_arrival_payment_link' => 'Le lien de paiement pour effectuer votre paiement.',
    'process_arrival_payment_required' => 'Paiement requis : Pour le fret maritime, vous devez régler 50% du montant global dans les 4 jours pour que votre colis soit inclus dans la prochaine expédition.',
    'process_arrival_storage_fee' => 'Frais de stockage : Tout colis restant plus de 7 jours dans notre entrepôt sans paiement sera facturé 1 USD (R18) par Kg ou CBM par jour à partir du 8e jour.',

    // contact page
    'contact_page_title' => 'Contactez-nous',
    'pages' => 'Pages',
    'contact_page_breadcrumb' => 'Contact',
    'contact_general_info_title' => 'Informations générales',
    'contact_general_info_paragraph' => 'TCC (Trusted Cargo) est opérationnel depuis l\'Afrique du Sud (Johannesburg) ainsi que depuis la Chine (Guangzhou et Shenzhen). Les envois sont acheminés vers quatre destinations finales : vers Johannesburg (pour les colis en provenance de Chine) et vers la République Démocratique du Congo (RDC) : Kinshasa, Lubumbashi et Kolwezi (pour les colis en provenance soit de Chine, soit d\'Afrique du Sud).',
    'contact_instructions_title_2' => 'Instructions pour vos fournisseurs en Chine',
    'contact_instructions_title_1' => 'Instructions pour les colis de Johannesburg à Kinshasa et/ou à Lubumbashi',
    'contact_instructions_p1' => 'Veuillez télécharger ci-dessous l\'image correspondant à votre ville de destination finale dans la rubrique et l\'envoyer à votre fournisseur. Cette image contient toutes les informations de livraison.',
    'contact_instructions_p2' => '',
    'contact_jhb_to_drc_title' => 'Instructions pour les colis de Johannesburg à Kinshasa et/ou à Lubumbashi',
    'contact_jhb_to_drc_intro' => 'Lors de vos achats sur des sites web sud-africains ou même chinois mais qui livrent à Johannesburg, utilisez les informations de livraison suivantes :',
    'contact_jhb_to_drc_table_field' => 'Champ',
    'contact_jhb_to_drc_table_format' => 'Format à utiliser',
    'contact_jhb_to_drc_field_buyer' => 'Acheteur (là où ça demande vos noms)',
    'contact_jhb_to_drc_format_buyer' => 'TCC + VILLE FINALE + VOS NOMS',
    'contact_jhb_to_drc_format_buyer_example' => 'Exemple : TCC LUSHI PATRICE LUMUMBA',
    'contact_jhb_to_drc_field_address' => 'Adresse de livraison',
    'contact_jhb_to_drc_format_address' => '30 Gaunt Avenue Bryanston Sandton Johannesburg 2191 South Africa',
    'contact_jhb_to_drc_field_phone' => 'Téléphone de contact pour livrer à Johannesburg',
    'contact_jhb_to_drc_format_phone' => '+27 73 298 53 11',
    'contact_jhb_to_drc_field_email' => 'Adresse email',
    'contact_jhb_to_drc_format_email' => 'Insérer votre propre adresse e-mail (si demandé) mais rassurez-vous de lire vos mails régulièrement.',
    'contact_jhb_to_drc_example_title' => 'Exemple pour l’acheteur qui attend son colis à Kinshasa :',
    'contact_jhb_to_drc_example_buyer' => 'Acheteur : TCC KINSHASA PATRICE LUMUMBA',
    'contact_jhb_to_drc_example_address' => 'Adresse de livraison : 30 Gaunt Avenue Bryanston Sandton Johannesburg 2191 South Africa',
    'contact_jhb_to_drc_example_phone' => 'Téléphone de contact : +27 73 298 53 11',
    'contact_jhb_to_drc_example_email' => 'Email : mettez votre propre adresse email si c’est demandé',
    'contact_jhb_to_drc_note' => 'NB : En adhérant strictement à ces directives de livraison, vous vous assurez que vos marchandises sont consolidées efficacement et préparées pour un transport sûr et fiable vers la RDC. Le non-respect des informations de livraison correctes et complètes pourrait entraîner des retards ou l\'égarement de vos articles.',
    'contact_city_jhb' => 'Johannesburg',
    'contact_city_jhb_desc' => 'Instructions d\'expédition pour Jhb',
    'contact_city_kin' => 'Kinshasa',
    'contact_city_kin_desc' => 'Instructions d\'expédition pour Kin',
    'contact_city_lub' => 'Lubumbashi',
    'contact_city_lub_desc' => 'Instructions d\'expédition pour Lub',
    'contact_city_kwz' => 'kolwezi',
    'contact_city_kwz_desc' => 'Instructions d\'expédition pour Kwz',
    'contact_download_file' => 'Télécharger le fichier',
    'contact_pickup_points_title' => 'Points de Récupération',
    'contact_city_jhb_full' => 'Johannesburg, Afrique du Sud',
    'contact_city_kin_full' => 'Kinshasa, RDC',
    'contact_city_lub_full' => 'Lubumbashi, RDC',
    'contact_city_kwz_full' => 'kolwezi, DRC',
    'contact_email' => 'admin@trustedcargo.co.za',

    'contact_pickup_jhb_heading' => 'Pour Johannesburg en Afrique du Sud, vous récupérerez vos colis à :',
    'contact_pickup_jhb_warning' => 'ATTENTION : Cette adresse ci-dessous est uniquement pour le dépôt et le retrait de colis. Aucune visite ou démarche administrative sur place.',
    'contact_pickup_jhb_name' => 'Trusted Cargo Company',
    'contact_pickup_jhb_line1' => '30 Gaunt Road',
    'contact_pickup_jhb_line2' => 'Bryanston Sandton',
    'contact_pickup_jhb_line3' => 'Johannesburg 2191',
    'contact_pickup_jhb_admin_heading' => 'Voici nos coordonnées pour l\'administration et l\'accueil par rendez-vous des clients souhaitant des renseignements :',
    'contact_pickup_jhb_admin_name' => 'Trusted Cargo Company',
    'contact_pickup_jhb_admin_line1' => 'Legal Hub Office Park',
    'contact_pickup_jhb_admin_line2' => '411 Main Ave, Ferndale',
    'contact_pickup_jhb_admin_line3' => 'Randburg, 2125',
    'contact_pickup_jhb_admin_email' => 'Email : admin@trustedcargo.co.za',
    'contact_pickup_jhb_admin_phone' => 'Téléphone : +27 73 298 5311',
    'contact_pickup_jhb_admin_note' => 'NB : Veuillez noter qu\'il est recommandé de prendre rendez-vous avant de vous présenter à nos bureaux.',

    'contact_pickup_kin_heading' => 'Pour Kinshasa en République démocratique du Congo, vous récupérerez vos colis à :',
    'contact_pickup_kin_name' => 'Trusted Cargo Company',
    'contact_pickup_kin_line1' => '14, Avenue Sergent Moke',
    'contact_pickup_kin_line2' => 'Commune de Ngaliema Kinshasa',
    'contact_pickup_kin_line3' => 'Réf. : Rond Point Socimat',
    'contact_pickup_kin_phone' => '+243 987 020 110',

    'contact_pickup_lub_heading' => 'Pour Lubumbashi en République démocratique du Congo, vous récupérerez vos colis à :',
    'contact_pickup_lub_name' => 'Trusted Cargo Company',
    'contact_pickup_lub_line1' => '108, Avenue Kasaï',
    'contact_pickup_lub_line2' => 'Commune de Lubumbashi',
    'contact_pickup_lub_line3' => 'Centre-ville',
    'contact_pickup_lub_line4' => 'Réf. : Cliniques universitaires',
    'contact_pickup_lub_phone' => '+243 852 934 920',

    'contact_pickup_kwz_heading' => 'Pour Kolwezi en République démocratique du Congo, vous récupérerez vos colis à :',
    'contact_pickup_kwz_name' => 'Trusted Cargo Company',
    'contact_pickup_kwz_note' => 'NB : Le retrait des colis pour Kolwezi se fait auprès de nos partenaires chinois sur place dans Kolwezi. L’adresse sera communiquée au cas par cas lors du retrait. Toute assistance nécessaire pour les clients de Kolwezi doit se faire avec notre bureau de Lubumbashi.',
    'contact_pickup_kwz_phone' => '+243 852 934 920',


    'contact_hours_title' => 'Horaires d\'ouverture',
    'contact_hours_weekdays_label' => 'Lundi à Vendredi :',
    'contact_hours_weekdays' => '08h30 - 17h00',
    'contact_hours_saturday_label' => 'Samedi :',
    'contact_hours_saturday' => '09h00 - 13h30',
    'contact_hours_holidays_label' => 'Jours fériés :',
    'contact_hours_holidays' => 'Ouvrable sur rendez-vous',
    'contact_hours_holiday_note' => 'Pour les jours fériés, veuillez nous contacter au préalable pour confirmer votre rendez-vous.',
    // contact form
    'contact_form_title' => 'Contactez-nous',
    'contact_form_description' => 'Le formulaire de contact est actuellement inactif. Obtenez un formulaire de contact fonctionnel et opérationnel avec Ajax & PHP en quelques minutes.',
    'contact_form_download' => 'Télécharger maintenant',
    'contact_form_name_placeholder' => 'Votre nom',
    'contact_form_name_label' => 'Votre nom',
    'contact_form_email_placeholder' => 'Votre email',
    'contact_form_email_label' => 'Votre email',
    'contact_form_phone_placeholder' => 'Téléphone',
    'contact_form_phone_label' => 'Votre téléphone',
    'contact_form_project_placeholder' => 'Projet',
    'contact_form_project_label' => 'Votre projet',
    'contact_form_subject_placeholder' => 'Sujet',
    'contact_form_subject_label' => 'Sujet',
    'contact_form_message_placeholder' => 'Laissez un message ici',
    'contact_form_message_label' => 'Message',
    'contact_form_send_button' => 'Envoyer le message',
    'contact_block_addresses' => 'Adresses',
    'contact_block_mobile' => 'Mobile',
    'contact_block_email' => 'Email',

    // buttons
    'button_info' => 'Information importante !',
    'text_info' => 'Cette fonctionnalité sera bientôt disponible. Nous y travaillons encore. En attendant, vous pouvez expédier vos colis directement à nos entrepôts de Johannesburg (pour les envois aériens vers la RDC) ou de Chine (pour les envois maritimes vers la RDC et Johannesburg). Ensuite, contactez notre entrepôt dans votre ville pour que nous puissions suivre votre colis. Rendez-vous dans la section « Nos contacts et entrepôts » pour commencer.',
    // terms and conditions
    'terms_title' => 'Termes et Conditions',

    'terms_intro' => 'Votre colis peut rester dans notre entrepôt de la ville de destination finale 40 jours maximum.
                Si nous n\'avons pas reçu votre paiement de la facture, nous considérons que vous avez abandonné ce(s)
                colis et que nous avons votre accord pour la destruction ou vente aux enchères.',
    'terms_title1' => 'Responsabilités de l\'expéditeur',
    'terms_title2' => 'Non-respect du règlement',
    'terms_title3' => 'Exigences spécifiques',
    'terms_title4' => 'Droit de refus',
    'terms_title5' => 'Modalités de transport',
    'terms_title6' => 'Paiement et facturation',
    'terms_title7' => 'Limitation de responsabilité',
    'terms_title8' => 'Délais de livraison',
    'terms_title9' => 'Réclamations',
    'terms_title10' => 'Prescription',
    'terms_title11' => 'Compétence juridique',
    'terms_title12' => 'Acceptation des conditions',

    'terms_title1_content' => 'L\'expéditeur est SEUL responsable de la liste d\'inventaire établie, de sa déclaration en Effets
                        Personnels ou Commerciaux. En cas de refus des services douaniers ou gouvernementaux de la ville
                        de destination finale, le client paiera les surtaxations avant la livraison au destinataire.',
    'terms_title1_content_highlight' => 'Le contrôle du poids à l\'arrivée est le SEUL contrôle pris en compte.',
    'terms_title1_content1' => 'Le destinataire ne pourra en aucun cas faire référence à la liste établie par l\'expéditeur et non
                        contresignée par Trusted Cargo company de la ville de destination finale. L\'expéditeur certifie
                        que le contenu des colis n\'est ni dangereux pour le transport, ni prohibé par les autorités.',

    'terms_title2_content' => 'Le non-respect du règlement entraînera une amende et/ou une confiscation de la marchandise
                        illicite par les autorités en vigueur.',

    'terms_title3_content_sub_article_title1' => '1. Exigences pour produits textiles',
    'terms_title3_content_sub_article_text1' => 'Les produits textiles, surtout à destination de l\'Afrique du Sud, doivent être munis
                            d\'étiquettes d\'entretien cousues, d\'étiquettes de composition et d\'étiquettes « Fabriqué en
                            Chine ». Ces trois étiquettes sont obligatoires pour tous les produits conformes aux normes
                            et qualifiés.',
    'terms_title3_content_sub_article_title2' => '2. Restrictions dimensionnelles pour l\'expédition',
    'terms_title3_content_sub_article_text2' => 'Pour les marchandises dépassant 3 mètres de longueur ou 1 000 kg, veuillez nous consulter
                            séparément. Des frais de manutention supplémentaires, tels que ceux liés à un chariot
                            élévateur ou à une grue, peuvent s\'appliquer et seront facturés sur la base des coûts réels.',
    'terms_title3_content_sub_article_text2_list1' => 'Si un article pèse plus de 70 kg ou que son côté mesure plus de 180 cm, des frais de
                                transport hors gabarit de 35 USD (R 625) par envoi seront facturés (maximum de 10
                                articles par envoi ; un supplément de 8 USD (R125) par article sera facturé pour tout
                                excédent). Une base en bois de 9 cm (patin) est requise.',
    'terms_title3_content_sub_article_text2_list2' => 'Si un article pèse plus de 150 kg ou que son côté mesure plus de 200 cm, des frais de
                                transport hors gabarit de 70 USD (R 1250) par envoi seront facturés (maximum de 10
                                articles par envoi ; un supplément de 10 USD (R170) par article sera facturé pour tout
                                excédent). Une base en bois de 9 cm (patin) est requise.',

    'terms_title4_content1' => 'L\'expéditeur se réserve le droit de refuser certaines marchandises (entre autres : animaux,
                        bijoux, mandats postaux, documents d\'identité, obligations, chèques, devises, traites, titres,
                        chèques de voyage, ...), de ne pas manipuler des biens dangereux, d\'interdire ceux qui sont
                        déterminés notamment par les lois et les dispositions nationales et internationales.',
    'terms_title4_content2' => 'La marchandise est prise en charge sans vérification de son contenu et de l\'état de celui-ci. Ni
                        la vérification des indications fournies, ni celles des poids déclarés par le client ne sont
                        obligatoires pour l\'expéditeur. L\'expéditeur ou les autorités compétentes ont, si nécessaire, le
                        droit de vérifier le contenu des envois.',
    'terms_title4_content3' => 'Par ailleurs, le client garantit que la marchandise est emballée pour assurer un transport sûr
                        vis à vis des soins ordinaires de manutention.',

    'terms_title5_content1' => 'L\'expéditeur jouit d\'une totale liberté pour déterminer la manière et les moyens par lesquels les
                        biens seront transportés. Tous les renseignements nécessaires (indications d\'expédition telles
                        qu\'instruites, etc., ...) doivent figurer clairement sur tout envoi.',
    'terms_title5_content2' => 'En cas de refus des marchandises par le destinataire ou de défaillance de ce dernier pour quelque
                        cause que ce soit, le client prendra en charge tous les frais afférents au changement de
                        destination de la marchandise ou éventuellement à son rapatriement et son stockage.',


    'terms_title6_content1' => 'Toutes les factures sont payables dès leur réception par le client, c\'est-à-dire au guichet de
                        l\'expéditeur ou via le lien de paiement qui lui sera envoyé, avant l\'envoi.',
    'terms_title6_content2' => 'Si les tarifs pour le transport et les autres services ne sont pas payés avant I\'envoi, tous les
                        frais seront payés dans les 5 jours suivants la réception de la facture, laquelle sera
                        considérée avoir été reçue 3 jours ouvrables après la date de la facture, sauf preuve contraire.',
    'terms_title6_content3' => 'Si des droits, taxes, amendes, frais ou dépenses quelconques sont imposés ou encourus en raison
                        d\'une action des douanes ou du manquement de l\'expéditeur ou du destinataire à ses obligations
                        de fournir les documents appropriés ou toute licence ou permis requis en rapport avec le
                        transport, ils seront préalablement imputés au destinataire et, en l\'absence de paiement de ce
                        dernier, il incombera alors au client de payer le montant en question sur simple demande.',
    'terms_title6_content_highlight' => 'Les sommes impayées à leur date d\'exigibilité porteront d\'office et sans mise en demeure
                            préalable intérêt au taux annuel de 12% majoré d\'une indemnité forfaitaire équivalente à 15%
                            de la dette avec un minimum de 50 USD (R900), destiné à couvrir le dommage économique et
                            administratif subi, sans préjudice du droit de l\'expéditeur de prouver l\'existence d\'un dommage plus important.',
    'terms_title6_content4' => 'L\'expéditeur a un privilège sur toutes les marchandises transportées pour le fret, les droits de douane, les sommes avancées et toutes autres charges résultant du transport. L\'expéditeur a en outre un droit de rétention sur les marchandises et est en droit de les vendre afin de couvrir totalement ses frais, après mise en demeure demeurée sans suite pendant 15 jours.',


    'terms_title7_content1' => 'L\'expéditeur ne peut être tenu que de ses fautes et négligences dans le cadre de l\'exécution de
                        sa mission pour autant que celles-ci aient causé un dommage matériel ou financier direct dans le
                        chef du client, à l\'exclusion de tout dommage indirect, notamment économique.',
    'terms_title7_content2' => 'L\'expéditeur n\'assume aucune responsabilité si le dommage subi est, ne fût ce qu\'en partie, dû à
                        un cas de Force majeure ou à la faute d\'un tiers ou du client.',
    'terms_title7_content_highlight' => 'Lorsqu\'elle est engagée, la responsabilité de l\'expéditeur est limitée, sauf convention
                            expresse contraire, à 5 USD (R 90) par kilo de poids brut avarié ou perdu, avec un maximum
                            de 25.000 USD (R 450.000) par mission.',

    'terms_title8_content' => 'Les délais de livraison ne sont pas garantis par L\'expéditeur, sauf accord contraire préalable et
                        écrit. La simple mention par le client d\'un délai de livraison n\'engage en rien l\'expéditeur.',


    'terms_title9_content1' => 'Pour être recevable, toute réclamation doit être adressée à l\'expéditeur par lettre recommandée dans les 14 jours qui suivent l\'envoi ou la livraison.',
    'terms_title9_content2' => 'Si le destinataire néglige de prendre livraison, le délai de 14 jours prend cours à la mise à disposition des marchandises concernées auprès de la personne habilitée à en recevoir la livraison.',

    'terms_title10_content' => 'Toute action en responsabilité à l\'encontre de l\'expéditeur est éteinte par prescription si elle
                        n\'est pas introduite devant un tribunal compétent dans un délai de 6 mois à compter du jour qui
                        suit le jour où les marchandises ont ou auraient dû être livrées ou, à défaut, à compter du jour
                        qui suit le jour où s\'est produit le Fait qui entraîne la réclamation.',


    'terms_title11_content1' => 'Seuls les tribunaux de la circonscription judiciaire de l\'entrepôt de la ville de destination
                        finale du colis sont compétents, sans préjudice du droit de l\'expéditeur d\'introduire le litige
                        devant une autre juridiction.',
    'terms_title11_content2' => 'Les présentes conditions, tout contrat conclu qui incorpore celles-ci, et de manière générale,
                        l\'intégralité des relations entre l\'expéditeur et le client, seront régis par le droit soit
                        sudafricain pour l\'entrepôt situé en Afrique du Sud, et congolais pour l\'entrepôt situé en
                        République démocratique du Congo.',


    'terms_title12_content1' => 'En plaçant et en payant cette Facture, vous acceptez les termes et les conditions d\'utilisation
                        de Trusted Cargo Company.',
    'terms_title12_content2' => 'Toutes nos opérations sont soumises aux conditions générales inscrites dans ce cadre.',
    'terms_title12_content_highlight' => 'Nos contractants reconnaissent avoir parfaite connaissance desdites conditions et déclarent
                            les avoir acceptées.',

    // payment section
    'payment_title' => 'Paiement',
    'payment_text' => 'Paiement de vos expéditions',
    'payment_info_text' => 'Alors que de façon générale TCC évaluera votre colis depuis notre entrepôt en Chine et vous enverra le lien par mail ou par WhatsApp pour payer pour votre expédition, vous pouvez toujours effectuer également votre paiement ici en ligne en insérant le montant ainsi que le numéro de référence de votre expédition afin que nous puissions identifier l’expédition pour laquelle vous payez.',
    'payment_feature1' => 'Insérez le montant et le numéro de référence de votre expédition',
    'payment_feature2' => 'Nous identifions l\'expédition pour laquelle vous payez',
    'payment_feature3' => 'Processus sécurisé et rapide',
    'payment_currency_note' => 'Le terminal vous facturera en Rand sudafricain, et la conversion se fera automatiquement par votre banque si votre compte bancaire est dans une devise différente du Rand sudafricain.',
    'payment_text_process' => 'Procéder au paiement',
    'payment_pay_button' => 'Payer maintenant',
    'payment_secure_text' => 'Paiement sécurisé avec chiffrement SSL',


    'unexpected_error' => 'Une erreur est survenue lors de la création.',
    'success' => 'Votre envoi a été créé avec succès.',
    'login_failed' => 'Échec de la connexion.',
    'please_fill_all_fields' => 'Veuillez remplir tous les champs du dossier !',
    'please_add_at_least_one_expedition' => 'Veuillez ajouter au moins une expédition !',
    'dossier_created_success' => 'Dossier créé avec succès !',

    // tracking result
    'tracking_result_title' => 'Historique des commandes',
    'tracking_result_subtitle' => 'Suivez vos commandes, consultez les statuts et les détails de livraison.',
    'tracking_result_last_sync' => 'Dernière mise à jour',
    'tracking_result_tracking_label' => 'Numéro de suivi',
    'tracking_result_missing' => 'Veuillez saisir un numéro de suivi.',
    'tracking_result_not_found' => 'Aucun dossier trouvé pour ce numéro de suivi.',
    'tracking_result_customer_id' => 'Référence client',
    'tracking_result_status_history' => 'Historique des statuts',
    'tracking_result_no_history' => 'Aucun historique disponible',
    'tracking_result_no_history_desc' => 'Aucun événement n\'a encore été enregistré.',
    'tracking_result_no_shipments' => 'Aucune expédition trouvée',
    'tracking_result_by' => 'par',
    'tracking_result_status_update' => 'Mise à jour du statut.',
    'tracking_result_prompt' => 'Entrez un numéro de suivi pour afficher l\'historique.',
    'tracking_result_details' => 'Détails',
    'tracking_result_summary_title' => 'Résumé',
    'tracking_result_total_shipments' => 'Total des expéditions',
    'tracking_result_period_all' => 'Toutes périodes',
    'tracking_result_quick_actions' => 'Actions rapides',
    'tracking_result_action_new_tracking' => 'Nouveau suivi',
    'tracking_result_action_new_tracking_desc' => 'Rechercher un autre numéro',
    'tracking_result_action_contact' => 'Support',
    'tracking_result_action_contact_desc' => 'Ouvrir un ticket ou appeler',
    'tracking_result_action_contact_short' => 'Aide',
    'tracking_order_label' => 'Commande',
    'tracking_filter_search' => 'Rechercher',
    'tracking_filter_status' => 'Statut',
    'tracking_filter_period' => 'Période',
    'tracking_filter_reset' => 'Réinitialiser',
    'tracking_filter_apply' => 'Appliquer',
    'tracking_filter_all' => 'Tous',
    'tracking_filter_pending' => 'En attente',
    'tracking_filter_in_progress' => 'En cours',
    'tracking_filter_delivered' => 'Livrée',
    'tracking_filter_cancelled' => 'Annulée',
    'tracking_filter_30_days' => '30 derniers jours',
    'tracking_filter_3_months' => '3 derniers mois',
    'tracking_filter_year' => 'Cette année',
    'tracking_filter_all_time' => 'Toutes',
    'tracking_summary_pending_desc' => 'En attente de traitement',
    'tracking_summary_in_progress_desc' => 'Préparation / expédition',
    'tracking_summary_delivered_desc' => 'Historique complété',
    'tracking_summary_cancelled_desc' => 'Expéditions annulées',
    'origin_label' => 'Origine',
    'destination_label' => 'Destination',
    'created_at_label' => 'Date création',
    'updated_at_label' => 'Dernière mise à jour',
    'description_label' => 'Description',
    'comment_label' => 'Commentaire',

];
